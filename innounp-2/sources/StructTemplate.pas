...
const
  ...
  SetupHeaderOptionTable: array[0..MySetupHeaderOptionLast] of byte = (
    ...
    {$IF DEFVER<6400}ord(structDEFVER_EXTSUF_.shWindowVisible)             {$ELSE}255{$IFEND},
    {$IF DEFVER<6400}ord(structDEFVER_EXTSUF_.shWindowShowCaption)         {$ELSE}255{$IFEND},
    {$IF DEFVER<6400}ord(structDEFVER_EXTSUF_.shWindowResizable)           {$ELSE}255{$IFEND},
    {$IF DEFVER<6400}ord(structDEFVER_EXTSUF_.shWindowStartMaximized)      {$ELSE}255{$IFEND},
    ...
    {$IF DEFVER<6700}ord(structDEFVER_EXTSUF_.shUsePreviousAppDir)          {$ELSE}255{$IFEND},
    {$IF DEFVER<6400}ord(structDEFVER_EXTSUF_.shBackColorHorizontal)        {$ELSE}255{$IFEND},
    {$IF DEFVER<6700}ord(structDEFVER_EXTSUF_.shUsePreviousGroup)           {$ELSE}255{$IFEND},
    ord(structDEFVER_EXTSUF_.shUpdateUninstallLogAppName),
    {$IF (DEFVER>=2001) AND (DEFVER<6700)}ord(structDEFVER_EXTSUF_.shUsePreviousSetupType)      {$ELSE}255{$IFEND},
    ...
    {$IF (DEFVER>=2001) AND (DEFVER<6700)}ord(structDEFVER_EXTSUF_.shUsePreviousTasks)          {$ELSE}255{$IFEND},
    ...
    {$IF (DEFVER>=3000) AND (DEFVER<6700)}ord(structDEFVER_EXTSUF_.shUsePreviousUserInfo)       {$ELSE}255{$IFEND},
    ...
    {$IF (DEFVER>=6600) AND (DEFVER<6700)}ord(structDEFVER_EXTSUF_.shWizardLightButtonsUnstyled){$ELSE}255{$IFEND},
    {$IF DEFVER>=6700}ord(structDEFVER_EXTSUF_.shRedirectionGuard)         {$ELSE}255{$IFEND},
    {$IF DEFVER>=6700}ord(structDEFVER_EXTSUF_.shWizardBevelsHidden)       {$ELSE}255{$IFEND},
    {$IF DEFVER>=6700}ord(structDEFVER_EXTSUF_.shUnusedPadding)            {$ELSE}255{$IFEND}
  );
