...

{
  ...
  $jrsoftware: issrc/Projects/Struct.pas,v 1.247 2010/02/18 13:33:57 mlaan Exp $
}
...
const
  ...
  SetupVersion = '5.3.8 '{$IFDEF ISUNICODE}+'(u)'{$ELSE}+'(a)'{$ENDIF};
  SetupBinVersion = (5 shl 24) + (3 shl 16) + (8 shl 8) + 0;
...
const
  ...
  SetupID: TSetupID = 'Inno Setup Setup Data (5.3.8)'{$IFDEF ISUNICODE}+' (u)'{$ENDIF};
  UninstallLogID: array[Boolean] of TUninstallLogID =
    ('Inno Setup Uninstall Log (b)', 'Inno Setup Uninstall Log (b) 64-bit');
  MessagesHdrID: TMessagesHdrID = 'Inno Setup Messages (5.1.11)'{$IFDEF ISUNICODE}+' (u)'{$ENDIF};
...
type
  ...
  TSetupHeaderOption = (shDisableStartupPrompt, shUninstallable, shCreateAppDir,
    ...
    {$IFNDEF ISUNICODE}shShowUndisplayableLanguages, {$ENDIF}shSetupLogging,
    shSignedUninstaller, shUsePreviousLanguage);
...
const
  SetupHeaderStrings = 25;
  SetupHeaderAnsiStrings = 5;
type
  TSetupHeader = packed record
    AppName, AppVerName, AppId, AppCopyright, AppPublisher, AppPublisherURL,
      ...
      AppModifyPath, CreateUninstallRegKey: WideString;
      LicenseText, InfoBeforeText, InfoAfterText, SignedUninstallerSignature,
...
{$ENDIF}
    Name, LanguageName, DialogFontName, TitleFontName, WelcomeFontName,
      CopyrightFontName: WideString;
    Data, LicenseText, InfoBeforeText, InfoAfterText: AnsiString;
    LanguageID{$IFNDEF ISUNICODE}, LanguageCodePage{$ENDIF}: Cardinal;
...
type
  ...
  TSetupTypeEntry = packed record
    Name, Description, Languages, Check: WideString;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
...
type
  ...
  TSetupComponentEntry = packed record
    Name, Description, Types, Languages, Check: WideString;
    ExtraDiskSpaceRequired: Integer64;
    Level: Integer;
    Used: Boolean;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
...
type
  ...
  TSetupTaskEntry = packed record
    Name, Description, GroupDescription, Components, Languages, Check: WideString;
    Level: Integer;
    Used: Boolean;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
...
type
  ...
  TSetupDirEntry = packed record
    DirName: WideString;
    Components, Tasks, Languages, Check, AfterInstall, BeforeInstall: WideString;
    Attribs: Integer;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
...
type
  ...
  TSetupFileEntry = packed record
    SourceFilename, DestName, InstallFontName, StrongAssemblyName: WideString;
    Components, Tasks, Languages, Check, AfterInstall, BeforeInstall: WideString;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
...
type
  ...
  TSetupIconEntry = packed record
    IconName, Filename, Parameters, WorkingDir, IconFilename, Comment: WideString;
    Components, Tasks, Languages, Check, AfterInstall, BeforeInstall: WideString;
    AppUserModelID: WideString;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
...
type
  ...
  TSetupIniEntry = packed record
    Filename, Section, Entry, Value: WideString;
    Components, Tasks, Languages, Check, AfterInstall, BeforeInstall: WideString;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
...
type
  ...
  TSetupRegistryEntry = packed record
    Subkey, ValueName, ValueData: WideString;
    Components, Tasks, Languages, Check, AfterInstall, BeforeInstall: WideString;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
    RootKey: HKEY;
    PermissionsEntry: Smallint;
...
type
  ...
  TSetupRunEntry = packed record
    Name, Parameters, WorkingDir, RunOnceId, StatusMsg, Verb: WideString;
    Description, Components, Tasks, Languages, Check, AfterInstall, BeforeInstall: WideString;
    MinVersion, OnlyBelowVersion: TSetupVersionData;
    ShowCmd: Integer;
