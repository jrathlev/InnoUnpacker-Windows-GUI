...
const
  ...
  SetupVersion = '5.5.6 '{$IFDEF ISUNICODE}+'(u)'{$ELSE}+'(a)'{$ENDIF};
  SetupBinVersion = (5 shl 24) + (5 shl 16) + (6 shl 8) + 0;
...
const
  ...
  SetupID: TSetupID = 'Inno Setup Setup Data (5.5.6)'{$IFDEF ISUNICODE}+' (u)'{$ENDIF};
  UninstallLogID: array[Boolean] of TUninstallLogID =
    ('Inno Setup Uninstall Log (b)', 'Inno Setup Uninstall Log (b) 64-bit');
  MessagesHdrID: TMessagesHdrID = 'Inno Setup Messages (5.5.3)'{$IFDEF ISUNICODE}+' (u)'{$ENDIF};
...
type
  TSetupHeader = packed record
    AppName, AppVerName, AppId, AppCopyright, AppPublisher, AppPublisherURL,
      ...
      CloseApplicationsFilter, SetupMutex: WideString;
