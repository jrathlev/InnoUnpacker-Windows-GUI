...

{
  ...
  $jrsoftware: issrc/Projects/zlib.pas,v 1.8 2003/05/25 06:03:04 jr Exp $
}
...
type
  TBlockDeflateHeader = packed record
...
{  TCompressorWriteProc = procedure(const Buffer; Count: Longint) of object;
  ...
  public
    constructor Create(AWriteProc: TCompressorWriteProc; CompressionLevel: Integer); virtual;
  ...
  public
    constructor Create(AWriteProc: TCompressorWriteProc; CompressionLevel: Integer); override;
...
type
  ...
  EZlibDataError = class(EZlibError);
  EZlibInternalError = class(EZlibError);
...
begin
  ...
  for I := Low(ValidCodes) to High(ValidCodes) do
    if ValidCodes[I] = Code then
      Exit;
  raise EZlibInternalError.CreateFmt(SZlibInternalError, [Code]);
end;
    ...
    try
      ...
      try
        ...
        repeat
          ...
          case Res of
            Z_OK, Z_STREAM_END: ;
            Z_DATA_ERROR: raise EZlibDataError.Create(SZlibDataError);
          else
            raise EZlibInternalError.CreateFmt(SZlibInternalError, [Res]);
          end;
          if strm.total_out <> 0 then begin
...
begin
  with Data do
    if (OnlyIfFull and (strm.avail_out = 0)) or
      ...
      CRC := GetCRC32(OutBuffer, SizeOf(OutBuffer) - strm.avail_out);
      BlockWrite(F^, CRC, SizeOf(CRC));
      BlockWrite(F^, OutBuffer, SizeOf(OutBuffer) - strm.avail_out);
      strm.next_out := @OutBuffer;
      strm.avail_out := SizeOf(OutBuffer);
    end;
end;
...
begin
  with Data do begin
    ...
    Seek(F^, StartPos);
    if Compressed then
      Hdr.CompressedSize := strm.total_out
    else
      Hdr.CompressedSize := -1;
    Hdr.UncompressedSize := strm.total_in;
...
begin
  ...
  if HdrCRC <> GetCRC32(Hdr, SizeOf(Hdr)) then
    raise EZlibDataError.Create(SZlibDataError);
  if Hdr.CompressedSize <> -1 then begin
  ...
  with Data do begin
    ...
    while Left <> 0 do begin
      if (strm.avail_in = 0) and (InBytesLeft <> 0) then begin
        Len := InBytesLeft;
        if Len > SizeOf(InBuffer) then Len := SizeOf(InBuffer);
        if F.Position.Lo + SizeOf(CRC) + Len > F.CappedSize then
          raise EZlibDataError.Create(SZlibDataError);
        ...
        strm.avail_in := Len;
        if CRC <> Cardinal(GetCRC32(InBuffer, Len)) then
          raise EZlibDataError.Create(SZlibDataError);
      end;
...
constructor TZCompressor.Create(AWriteProc: TCompressorWriteProc;
  CompressionLevel: Integer);
begin
  inherited Create(AWriteProc, CompressionLevel);
  InitStream(FStrm);
...
begin
  ...
  while FStrm.avail_out > 0 do begin
    ...
    case Check(inflate(FStrm, Z_NO_FLUSH), [Z_OK, Z_STREAM_END, Z_DATA_ERROR, Z_BUF_ERROR]) of
      ...
      Z_DATA_ERROR, Z_BUF_ERROR: raise EZlibDataError.Create(SZlibDataError);
    end;
  end;
end;
