...
const
  ...
  SetupVersion = '6.0.3 '{$IFDEF ISUNICODE}+'(u)'{$ELSE}+'(a)'{$ENDIF};
...
const
  ...
  SetupID: TSetupID = 'Inno Setup Setup Data (6.0.0)'{$IFDEF ISUNICODE}+' (u)'{$ENDIF};
  UninstallLogID: array[Boolean] of TUninstallLogID =
    ('Inno Setup Uninstall Log (b)', 'Inno Setup Uninstall Log (b) 64-bit');
  MessagesHdrID: TMessagesHdrID = 'Inno Setup Messages (6.0.0)'{$IFDEF ISUNICODE}+' (u)'{$ENDIF};
