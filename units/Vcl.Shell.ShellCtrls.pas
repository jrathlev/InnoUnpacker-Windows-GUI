...
{                                                            }
{       Copyright (c) 1995,                                  }
{            2001-2015 Embarcadero Technologies Inc.         }
{                                                            }
{************************************************************}
{ Changes - J. Rathlev, kontakt(a)rathlev-home.de
  - Feb. 2008: Sort on column click for name, size, type and date
               Preselectable column widths (SetColWiths)
  - Jan. 2009: Added memory leak corrections in destroy methods
  - Apr. 2009: Added code for links to network resources
  - Oct. 2012: Added code to dispose PIDLs
  - Nov. 2012: Added all error corrections from
               http://www.kutinsoft.com/Hints/DelphiHints.php
  - Oct. 2015: Property to optionally exclude zip files from tree view
...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.ComCtrls, Winapi.CommCtrl, Winapi.ShlObj,
  Winapi.ActiveX, Vcl.StdCtrls, Vcl.ImgList, System.SyncObjs;

const
  WM_DEFFERED_REFRESH = WM_USER + 1986;
...
type
  ...
  TShellObjectType = (otFolders, otNonFolders, otHidden, otHiddenSystem);
  ...
  public
    ...
    function IsLink: Boolean;                        // JR - 2009-04-08
    function IsStream: Boolean;                      // JR - Oct 2015
...
{ TCustomShellTreeView }
  ...
  protected
    ...
    procedure DefferedRefreshEvent;   // JR - see http://www.kutinsoft.com/Hints/DelphiHints.php
...
{ TShellComboBox }
  ...
  TShellListSortCompare = function (ACol : integer; Reverse : boolean; Item1, Item2: Pointer): Integer;
  TShellList = class(TList)
    procedure Sort(ACol : integer; ARev : boolean; Compare: TShellListSortCompare);
    end;
...
{ TCustomShellListView }
  ...
  private
    ...
    FShowZip : boolean;              // JR - Oct 2017
    FSavePath: string;
    FCol: integer;
    FReverse: boolean;
    FColWidths:  array of word;
  ...
  public
    ...
    property ShellTreeView: TCustomShellTreeView read FTreeView write SetTreeView;
    property ShellComboBox: TCustomShellComboBox read FComboBox write SetComboBox;
    property ShowZip : boolean read FShowZip write FShowZip default false; // JR - Oct 2017
...
//  Flags := SFGAO_DISPLAYATTRMASK;
  ...
  // following line changed, J. Rathlev (kontakt(a)rathlev-home.de)
  // see Borland Quality Central Report #:  11641
  Flags := SFGAO_FILESYSTEM or SFGAO_FILESYSANCESTOR or SFGAO_REMOVABLE or SFGAO_VALIDATE;
  ParentFolder.GetAttributesOf(1, PIDL, Flags);
...
const
  NetLink = 'target.lnk';    // JR - 2009-04-14
  IID_IPersistFile: TGUID = '{0000010b-0000-0000-C000-000000000046}';

function GetLinkIdList (const LinkObj : string; var pidl : PItemIDList) : Boolean; // JR - 2009-04-14
var
  psl : IShellLink;
  ppf : IPersistFile;
  hr  : HResult;
...
function GetPathLinkIdList (const APath : string; var pidl : PItemIDList) : boolean;
var
  s : string;
begin
  s:=IncludeTrailingPathDelimiter(APath)+NetLink;
  Result:=FileExists(s) and GetLinkIdList(s,pidl);
  end;

function GetIsLink(Parentfolder: IShellFolder; PIDL: PItemIDList): Boolean;   // JR - 2009-04-08
var
  Flags: LongWord;
begin
  Flags := SFGAO_LINK;
  ParentFolder.GetAttributesOf(1, PIDL, Flags);
  Result:=IsElement(SFGAO_LINK, Flags);
  end;

function GetIsStream(Parentfolder: IShellFolder; PIDL: PItemIDList): Boolean;   // JR - Oct 2015
var
  Flags: LongWord;
begin
  if ParentFolder=nil then Result:=false
  else begin
    Flags := SFGAO_STREAM;
    ParentFolder.GetAttributesOf(1, PIDL, Flags);
    Result:=IsElement(SFGAO_STREAM, Flags);
    end;
  end;
...
begin
  ...
  { This is a hack bug fix to get around Windows Shell Controls returning
    spurious "?"s in date/time detail fields, changed for Unicode from '?' to #$200E - JR, 2009-02-20 }
  if (Length(Result) > 1) then begin
    if (Result[1] = #$200E) or (Result[1] = #$200F) then begin
      Result := StringReplace(Result,#$200E,'',[rfReplaceAll]);      // left-to-right-mark
      Result := StringReplace(Result,#$200F,'',[rfReplaceAll]);      // right-to left-mark
    end;
  end;
end;
...
begin
  ...
  if otHiddenSystem in ObjectTypes then Inc(Result, SHCONTF_ENABLE_INCLUDESUPERHIDDEN);
end;
...
begin
  ...
  DisposePIDL(NewPIDL);                         // JR
  if Assigned(RootFolder) then RootFolder.Free;
end;

{ ---------------------------------------------------------------- }
procedure QuickSort(SortList: PPointerList; ACol,L, R: Integer; ARev : boolean;
  SCompare: TShellListSortCompare);
var
  I, J: Integer;
  P, T: Pointer;
begin
  repeat
    ...
    repeat
      while SCompare(ACol,ARev,SortList^[I], P) < 0 do
        Inc(I);
      while SCompare(ACol,ARev,SortList^[J], P) > 0 do
        Dec(J);
      if I <= J then
      begin
        T := SortList^[I];
    ...
    until I > J;
    if L < J then
      QuickSort(SortList,ACol,L,J,ARev,SCompare);
    L := I;
  until I >= R;
end;

procedure TShellList.Sort(ACol : integer; ARev : boolean;Compare: TShellListSortCompare);
begin
  if (List <> nil) and (Count > 0) then
    QuickSort(@List, ACol, 0, Count - 1, ARev, Compare);
end;
...
function TShellFolder.IsLink: Boolean;              // JR - 2009-04-08
begin
  Result := GetIsLink(ParentShellFolder, FPIDL);
end;

function TShellFolder.IsStream: Boolean;           // JR - Oct 2015
begin
  Result := GetIsStream(ParentShellFolder, FPIDL);
end;
...
begin
  ...
  begin
    ...
    FThread := nil;
//    Temp.Terminate;
    Temp.Free;   // JR - see http://www.kutinsoft.com/Hints/DelphiHints.php
...
//  FreeOnTerminate := True;
  FreeOnTerminate := false;    // JR - see http://www.kutinsoft.com/Hints/DelphiHints.php
  ...
  FWaitChanged := false;
  // JR - see http://www.kutinsoft.com/Hints/DelphiHints.php
...
begin
//  ClearItems;  see https://forums.codegear.com/thread.jspa?messageID=48127
...
begin
  ...
  begin
    ...
    NewNode.HasChildren := IsFolder;      // see Borland report 41777 for changes
...
begin
  ...
  try
    ...
    begin
      if FShowZip or not GetIsStream(TShellFolder(Node.Data).ShellFolder,ID) then begin   // JR - Oct 2015
...
begin
  ...
  if Result then
    if Fldr.IsLink and GetPathLinkIdList(Fldr.PathName,pidl) then begin     // JR - 2009-04-08
      Fldr.Free;
      Node.Data:=TShellFolder.Create(nil,pidl,GetIShellFolder(nil,pidl));
      PopulateNode(Node);
      end
    else if Fldr.IsFolder and (Node.HasChildren) and (Node.Count = 0) then
      PopulateNode(Node)
...
procedure TCustomShellTreeView.DefferedRefreshEvent;
begin
  PostMessage(Handle, WM_DEFFERED_REFRESH, 0, 0);
end;

procedure TCustomShellTreeView.OnDefferedRefreshEvent(var Message: TMessage);
begin
  RefreshEvent;
end;
...
begin
  ...
  TempID := CopyPIDL(TShellFolder(TopItem.Data).RelativeID);
  if TShellFolder(TopItem.Data).Parent <> nil then begin     // JR Oct. 2012
    TopID := ConcatPIDLs(TShellFolder(TopItem.Data).Parent.AbsoluteID, TempID);
    DisposePIDL(TempID);
    end
  else TopID:=TempID;
  ...
  begin
    TempID := CopyPIDL(TShellFolder(Selected.Data).RelativeID);   // JR Oct. 2012
    if TShellFolder(Selected.Data).Parent <> nil then begin
      SelID := ConcatPIDLs(TShellFolder(Selected.Data).Parent.AbsoluteID, TempID);
      DisposePIDL(TempID);
      end
    else SelID:=TempID;
  end;
  ...
  try
    ...
    begin
      ...
      HasChildren := NewFolder.IsFolder;    // see Borland report 41777 for changes
//      HasChildren := NewFolder.SubFolders;
      Text := NewFolder.DisplayName;
    end;
    Selected:=NewNode;  // JR - 2012, to prevent from selection of the next folder
                        // when deleting node - see below
                        // (eg. node points to C:\ => node.delete will select D:\
                        // and prompt to insert a medium if this is a DVD drive)

    ThisLevel := Node.Level;
    OldNode := Node;
    if Node.Expanded then NewNode.Expand(False);    // JR - Nov. 2012
{    repeat                   // calls nested loops - long delay on large directories
...
begin
  ...
  begin
    ...
    if not Fldr.IsFolder then Fldr := Fldr.Parent;
    FUpdating := True;
    try
      if Assigned(FComboBox) then
...
begin
  ...
  if assigned(Selected) then Selected.MakeVisible;  // JR - Oct 2015
end;
...
function ListSortFunc(ACol : integer; Reverse : boolean; Item1, Item2: Pointer): Integer;
const
...
i: integer;
begin
  for i := 0 to Items.Count - 1 do if Assigned(Folders[i]) then Folders[I].Free;
  if assigned(FRootFolder) then FRootFolder.Free;
...
begin
  ...
  FAutoNavigate := True;
  FAutoRefresh := False;
  FFolders := TShellList.Create;
  FTreeView := nil;
  FUpdating := False;
  FSettingRoot := False;
  FShowZip := false;              // JR - Oct 2017
...
procedure TCustomShellListView.ColClick(Column: TListColumn);
begin
  ...
  try
    ...
    try
      ...
      begin
        ...
        CanAdd := ((otFolders in ObjectTypes) and AFolder.IsFolder)
          or ((otNonFolders in ObjectTypes) and not AFolder.IsFolder)
          or (FShowZip and AFolder.IsStream);  // JR Oct. 2017
        if CanAdd and Assigned(FOnAddFolder) then FOnAddFolder(Self, AFolder, CanAdd);
      ...
      begin
        ...
        try
          FFolders.Sort(FCol,FReverse,@ListSortFunc);
        finally
          CompareFolder := nil;
        end;
      end;
...
function TCustomShellListView.GetFolderCount : integer;
begin
...
// AWidth = number of characters for each column
procedure TCustomShellListView.SetColWidths(const AWidths : array of word);
var
  i : integer;
begin
...
//            n:=FColWidths[Columns.Count-1]
  ...
  begin
    if ColCount > 0 then begin
      if (Length(FColWidths)>0) and (FColWidths[0]>0) then n:=FColWidths[0] else n:=32;
      AddDefaultColumn(SShellDefaultNameStr, taLeftJustify, n);
      end;
    if ColCount > 1 then begin
      if (Length(FColWidths)>1) and (FColWidths[1]>0) then n:=FColWidths[1] else n:=15;
      AddDefaultColumn(SShellDefaultSizeStr, taRightJustify, n);
      end;
    if ColCount > 2 then begin
      if (Length(FColWidths)>2) and (FColWidths[2]>0) then n:=FColWidths[2] else n:=15;
      AddDefaultColumn(SShellDefaultTypeStr, taLeftJustify, n);
      end;
    if ColCount > 3 then begin
      if (Length(FColWidths)>3) and (FColWidths[3]>0) then n:=FColWidths[3] else n:=18;
      AddDefaultColumn(SShellDefaultModifiedStr, taLeftJustify, n);
      end;
  end;
...
procedure TCustomShellListView.DefferedRefreshEvent;
begin
  PostMessage(Handle, WM_DEFFERED_REFRESH, 0, 0);
end;

procedure TCustomShellListView.OnDefferedRefreshEvent(var Message: TMessage);
begin
  Refresh;
end;
...
function TCustomShellListView.GetPath : string;
begin
  Result:=FRootFolder.Pathname;
  end;
